...
​
# CD to project
cd ~/go-workspace/src/gitlab.com/dceu/monorepo
​
echo "******************************"
...
docker push registry.gitlab.com/dceu/monorepo/${APP}:${TAG}
wait
echo "Step 3: Uninstall pods ----------------------------------------------"
helm uninstall ${APP} --kube-context eks-${TAG} -n dceu
wait
echo "Step 4: Install pods ------------------------------------------------"
cd infrastructure/helm/${APP}/${TAG}
helm install ${APP} . --kube-context eks-${TAG} --set image.customTag=${TAG} -n dceu
wait
​
echo 'All done'
